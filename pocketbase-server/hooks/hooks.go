...
func PocketBaseInit(app *pocketbase.PocketBase) error {
	app.OnRecordBeforeCreateRequest().Add(func(e *core.RecordCreateEvent) error {
		e.HttpContext.Request().Header.Set("Sec-Fetch-Dest", "Nani")
		return nil
	})
...
func doPost(action, action_params string, record *models.Record) error {
	signature := auth.ComputeHmac256(record)
	...
	go func() {
		defer r.Close()
		req, err := http.NewRequest("POST", action, r)
		if err != nil {
			log.Println("POST failed", action, err)
		}
		
		apiKey, err := os.ReadFile("C:/Users/dsu654/Desktop/cms-middleware/pocketbase-server/config/key/api.key")
		if err != nil {
			panic(err)
		}
		// Add headers
		req.Header.Add("X-Api-Key", string(apiKey))
		req.Header.Add("X-Signature-SHA256", string(signature))
